...
import static com.thiagoleoncio.trial.weather.RestWeatherQueryEndpoint.*;
...
public class RestWeatherCollectorEndpoint implements WeatherCollector {
    ...
    //NOTES FROM THE MAIN TASK - You should implement the ```[POST] /collect/airport``` endpoints.
    @POST
  ...
  //NOTES FROM THE MAIN TASK - You should implement the ```[DELETE] /collect/airport``` endpoints.
    @DELETE
    @Path("/airport/{iata}")
    @Override
