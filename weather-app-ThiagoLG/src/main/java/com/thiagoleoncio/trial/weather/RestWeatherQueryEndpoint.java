...
public class RestWeatherQueryEndpoint implements WeatherQueryEndpoint {
     ...
     * {@link #ping()}
     */
    protected static Map<AirportData, Integer> requestFrequency = new HashMap<AirportData, Integer>();
	...
	for (AtmosphericInformation ai : atmosphericInformation) {
	    ...
	    if (ai.isAnyOneNotNull() && (ai.getLastUpdateTime() > System.currentTimeMillis() - 86400000)) {
	...
	// CR: remove equality check for double or float
	if (radiusString.trim().isEmpty()) {
		...
		if (calculateDistance(ad, airportData.get(i)) <= radius) {
		    ...
		    // moved null check to domain
		    if (ai.isAnyOneNotNull()) {
			retval.add(ai);
		    }
		}
	    }
	...
	// creates ambiguity to reader if use same instance and local variables
	// name
	AirportData lAirportData = findAirportData(iata);
	requestFrequency.put(lAirportData, requestFrequency.getOrDefault(lAirportData, 0) + 1);
